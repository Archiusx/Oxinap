...
import { saveInvestigation, subscribeRecentInvestigations, fetchRecentInvestigations, deleteInvestigation, updateInvestigation } from "./investigationStore";
...
function DashboardPage({ setActivePage, onStartInvestigation, onSelectInvestigation, investigation, investigationLoading, investigationError, recentItems, recentError, recentLoaded, onDeleteInvestigation, savingId, lastSavedId, user }) {
  ...
  const stats = [
    { label:"Total Investigations", value: isFirstLoad ? "—" : String(totalCount), delta: recentLoaded ? "Synced from Supabase" : "Loading…", icon:Target, color:"blue" },
        ...
        <div className="flex items-center justify-between px-5 py-4" style={V.inner}>
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color:"var(--text-muted)" }}>
              {recentLoaded ? `${totalCount} case${totalCount===1?"":"s"} on record` : isFirstLoad ? "Loading from Supabase…" : "Supabase sync error"}
            </p>
          </div>
          <div className="flex items-center gap-2">
...
const handleLogout = async () => {
  ...
  const handleStartInvestigation = async ({ target, type, redirectToOsint = true }) => {
    ...
    try {
      ...
      // Save to Supabase asynchronously — don't block UI on save
      saveInvestigation(user, result)
        .then(async () => {
          console.info("[CyIntel] Supabase save succeeded for case:", result.id);
          // Don't rely solely on the Realtime websocket (can lag/miss events
          // with Firebase-as-third-party-auth JWTs) — pull the fresh list
          // immediately so "Recent Investigations" updates right away.
          try {
            const fresh = await fetchRecentInvestigations(user);
            setRecentInvestigationsFromStore(fresh);
            setRecentInvestigationsLoaded(true);
          } catch (refreshErr) {
            console.error("[CyIntel] Manual refresh after save failed:", refreshErr);
          }
        })
        .catch((saveErr) => {
