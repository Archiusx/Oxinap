...
async function scrapeTwitterX(username) {
  ...
  // The Apify API token stays on the server (APIFY_API_TOKEN env var) and is
  // never bundled into client-side JS — only a validated username crosses the
  // network to our own backend.
  ...
  try {
    ...
    if (!res.ok) {
      ...
      // Surface a clear not-found if the actor returned no items (empty array)
      // or a 4xx that implies the account doesn't exist.
      if (res.status === 400) {
        return {
          platform: "Twitter / X",
          status: "not_found",
    ...
    // The actor also returns target metadata as the first item when
    // includeTargetMetadata is true — it has no `relation` field.
    // Use that to populate the profile card snippet.
    const meta = Array.isArray(followers)
      ? followers.find((f) => f.username?.toLowerCase() === cleanUsername.toLowerCase())
      : null;

    const followerCount  = meta?.follower_count  ?? (Array.isArray(followers) ? followers.length : 0);
    const followingCount = meta?.following_count ?? null;
    const description    = meta?.description     ?? "";
    const location       = meta?.location        ?? "";
    const displayName    = meta?.display_name    ?? cleanUsername;

    // Build a human-readable snippet from whatever metadata we got
    const snippetParts = [];
    ...
    return {
      platform: "Twitter / X",
      status: Array.isArray(followers) && followers.length > 0 ? "found" : "not_found",
      title: `@${cleanUsername}`,
      url: `https://x.com/${cleanUsername}`,
      snippet: snippetParts.join(" · ") || `Profile at x.com/${cleanUsername}`,
      metadata: {
        followers:  followerCount  != null ? String(followerCount)  : "Not public",
        following:  followingCount != null ? String(followingCount) : "Not public",
