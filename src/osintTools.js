...
async function scrapeTwitterX(username) {
  ...
  try {
    ...
    return {
      ...
      metadata: {
        profile_pic: target?.profile_pic_url || null,
...
async function scrapeLinkedIn(username) {
  ...
  try {
    ...
    return {
      ...
      metadata: {
        profile_pic: data.profilePicture || null,
        full_name: data.fullName || "Not public",
  ...
  try {
    ...
    return {
      ...
      metadata: {
        ...
        followers: data.followers != null ? String(data.followers) : "Not public",
        following: data.following != null ? String(data.following) : "Not public",
        public_repos: data.public_repos != null ? String(data.public_repos) : "Not public",
        twitter: data.twitter_username ? `@${data.twitter_username}` : "Not public",
        created_date: data.created_at ? data.created_at.slice(0, 10) : "Not public",
      },
...
export async function runPublicOsintInvestigation({ target, type, onProgress = null }) {
  ...
  // ── Stage 1 complete: fire progress callback so UI can show partial results ──
  if (typeof onProgress === "function") {
    const partialFoundCount = platformFindings.filter(f => f.status === "found").length;
    const partialConf = Math.min(60, 25 + partialFoundCount * 8);
    const partialRisk = partialConf >= 50 ? "medium" : "low";
    onProgress({
      findings: platformFindings,
      crawledPages: [...crawledPages],
      crawlErrors: [...crawlErrors],
      logs: [...logs],
      platforms: [...new Set(platformFindings.map(f => f.platform).filter(Boolean))].slice(0, 8),
      stats: { foundProfiles: partialFoundCount, candidateProfiles: 0, searchLinks: buildSearchLinks(normalizedTarget, detectedType).length, sources: crawledPages.length, crawledPages: crawledPages.length, confidence: partialConf },
      risk: partialRisk,
      gemini: null,
    });
  }
