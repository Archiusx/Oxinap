...
// Every Jina Reader call below tells the reader itself to give up after
// 10-12s (the "X-Timeout" header). Previously the *client* had no matching
// limit and fell through to fetchJson/fetchText's 90s default, so a single
// slow/hanging platform could stall the entire parallel batch for up to 90s
// even though the reader had already given up internally. This keeps a
// generous buffer over the reader's own timeout (network + cold start) while
// guaranteeing every scraper call surfaces a result in well under 20s.
const READER_TIMEOUT_MS = 18000;
...
async function githubLookup(username) {
  ...
  try {
    const data = await fetchJson(`https://api.github.com/users/${encodeURIComponent(username)}`, {}, 10000);
    return {
      platform: "GitHub",
      status: "found",
      title: data.login,
...
// Grounded search makes Gemini run real Google searches before it can answer,
// which routinely takes longer than a plain text generation call. The old
// 12s timeout was tight enough that a fully successful call could get cut off
// purely from being a little slow — this is the most likely cause of "works
// sometimes, not others" when the API key itself is configured correctly.
// Bumping the timeout, AND retrying once on transient failures (timeouts,
// network blips, and Gemini's own 429/5xx responses) makes the call much more
// consistent without ever changing what a fully successful response contains.
...
function isTransientGeminiError(err) {
  ...
  return /\b(429|500|502|503|504)\b/.test(msg) || /abort|timeout|network|fetch failed/i.test(msg);
}

async function callGeminiOnce(prompt, maxOutputTokens, apiKey) {
  return fetchJson(
...
async function askGemini(prompt, maxOutputTokens = 1400) {
  ...
  for (let attempt = 1; attempt <= GEMINI_MAX_ATTEMPTS; attempt++) {
    try {
      data = await callGeminiOnce(prompt, maxOutputTokens, apiKey);
      lastErr = null;
      break;
    } catch (err) {
      lastErr = err;
      if (attempt < GEMINI_MAX_ATTEMPTS && isTransientGeminiError(err)) {
...
export async function runPublicOsintInvestigation({ target, type }) {
  ...
  // WhatsMyName doesn't depend on anything Stage 1 produces (it only needs the
  // username), so fire it off now, in parallel with Stage 1, instead of in
  // Stage 2 where it previously waited for the whole platform-scraper batch
  // to finish before even starting. We only consume its result later.
  const whatsmynamePromise = username ? whatsmynameLookup(username) : Promise.resolve([]);
