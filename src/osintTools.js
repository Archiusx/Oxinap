...
async function lookupBTCAddress(address) {
  ...
  try {
    const [addrData, txsData] = await Promise.all([
      fetchJson(`https://blockstream.info/api/address/${encodeURIComponent(address)}`, {}, 12000),
      fetchJson(`https://blockstream.info/api/address/${encodeURIComponent(address)}/txs`, {}, 12000).catch(() => []),
    ]);
    ...
    try {
      const data = await fetchJson(`https://blockchain.info/rawaddr/${encodeURIComponent(address)}?limit=5&cors=true`, {}, 12000);
...
async function lookupETHAddress(address) {
  ...
  try {
    const [balRes, txRes, internalRes] = await Promise.all([
      fetchJson(`${baseUrl}?module=account&action=balance&address=${encodeURIComponent(address)}&tag=latest${keyParam}`, {}, 12000),
      fetchJson(`${baseUrl}?module=account&action=txlist&address=${encodeURIComponent(address)}&startblock=0&endblock=99999999&page=1&offset=10&sort=desc${keyParam}`, {}, 12000).catch(() => null),
      fetchJson(`${baseUrl}?module=account&action=tokentx&address=${encodeURIComponent(address)}&page=1&offset=5&sort=desc${keyParam}`, {}, 12000).catch(() => null),
    ]);
...
async function lookupTRXAddress(address) {
  try {
    const data = await fetchJson(`https://apilist.tronscanteam.com/api/account?address=${encodeURIComponent(address)}`, {}, 12000);
    ...
    return {
      ...
      title:    `TRX: ${address.slice(0, 8)}…${address.slice(-6)}`,
      url:      `https://tronscan.org/#/address/${address}`,
      snippet:  `TRX lookup failed: ${err.message}. Open Tronscan link to check manually.`,
      metadata: { chain: "Tron (TRX)", address },
      extractor: "Tronscan API",
    };
  }
}
...
async function lookupLTCAddress(address) {
  try {
    const data = await fetchJson(`https://api.blockcypher.com/v1/ltc/main/addrs/${encodeURIComponent(address)}/balance`, {}, 12000);
...
async function lookupDOGEAddress(address) {
  try {
    const data = await fetchJson(`https://api.blockcypher.com/v1/doge/main/addrs/${encodeURIComponent(address)}/balance`, {}, 12000);
...
export async function runCryptoWalletOsint({ address }) {
  ...
  const [chainResult, geminiResult, webSearch] = await Promise.allSettled([
    lookupFn(),
    cryptoGeminiSearch(addr, chain),
    runPublicReaderSearch(`${chain} wallet ${addr}`, "keyword"),
  ]);
  ...
  const crawledPages = [];
  if (webSearch.status === "fulfilled" && webSearch.value) {
    crawledPages.push(...webSearch.value.results.slice(0, 4));
    if (crawledPages.length) logs.push({ time: nowTime(), level: "info", msg: `Web search collected ${crawledPages.length} result(s).` });
  }
