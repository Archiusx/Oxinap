...
async function scrapeFacebook(username) {
  ...
  try {
    const res = await fetch("/api/scrape-facebook", { signal: AbortSignal.timeout(25000),
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
async function scrapeTwitterX(username) {
  ...
  try {
    const res = await fetch("/api/scrape-twitter", { signal: AbortSignal.timeout(25000),
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
async function scrapeLinkedIn(username) {
  ...
  try {
    const res = await fetch("/api/scrape-linkedin", { signal: AbortSignal.timeout(25000),
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export function detectCryptoChain(address) {
  ...
  if (/^bc1[a-z0-9]{6,87}$/i.test(addr)) return "BTC";
  if (/^0x[0-9a-fA-F]{40}$/.test(addr)) return "ETH";
  if (/^T[A-HJ-NP-Za-km-z1-9]{33}$/.test(addr)) return "TRX";
  if (/^[LM3][A-HJ-NP-Za-km-z1-9]{26,33}$/.test(addr)) return "LTC";
  if (/^ltc1[a-z0-9]{6,87}$/i.test(addr)) return "LTC";
  if (/^D[5-9A-HJ-NP-Up-z][1-9A-HJ-NP-Za-km-z]{32}$/.test(addr)) return "DOGE";
  if (/^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(addr) && !addr.startsWith("1") && !addr.startsWith("3")) return "SOL";
...
// Explorer links for a given chain
export function getCryptoExplorerLinks(address, chain) {
  const map = {
    BTC:  [ { name: "Blockstream",    url: `https://blockstream.info/address/${address}` },
             { name: "Mempool.space", url: `https://mempool.space/address/${address}` },
             { name: "Blockchain.com",url: `https://www.blockchain.com/explorer/addresses/btc/${address}` } ],
    ETH:  [ { name: "Etherscan",      url: `https://etherscan.io/address/${address}` },
             { name: "Blockchair",    url: `https://blockchair.com/ethereum/address/${address}` } ],
    TRX:  [ { name: "Tronscan",       url: `https://tronscan.org/#/address/${address}` } ],
    LTC:  [ { name: "Blockchair",     url: `https://blockchair.com/litecoin/address/${address}` } ],
    DOGE: [ { name: "Blockchair",     url: `https://blockchair.com/dogecoin/address/${address}` } ],
    SOL:  [ { name: "Solscan",        url: `https://solscan.io/account/${address}` },
             { name: "SolanaFM",      url: `https://solana.fm/address/${address}` } ],
    XRP:  [ { name: "XRPScan",        url: `https://xrpscan.com/account/${address}` } ],
  };
  return map[chain] || [];
}
