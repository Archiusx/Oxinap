...
async function cryptoGeminiSearch(address, chain) {
  ...
  // Lightweight Gemini call WITHOUT google_search grounding — avoids heavy network on mobile
  const prompt = `You are a blockchain OSINT analyst. Based on your training knowledge only (no web search), what do you know about this ${chain} wallet address: ${address}?
List any known entity labels, scam reports, exchange affiliations, or public associations. If unknown, say "No known associations in training data." Be concise, 3-5 sentences max.`;
  try {
    const data = await fetchJson(
      `https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL}:generateContent`,
      {
    ...
    return {
      enabled: true,
      summary: parseGeminiText(data) || "No known associations found.",
      sources: [],
      queries: [],
    };
  } catch { return null; }
