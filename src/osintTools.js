...
async function scrapeInstagram(username) {
  ...
  // Strategy 1 — direct page read via Jina Reader.
  try {
    const text = await fetchText(readerUrlFor(url), {
      headers: { Accept: "text/plain", "X-Return-Format": "markdown", "X-Timeout": "12" },
    });
    const followers = text.match(/(\d[\d,\.]+\s*[KkMm]?)\s*(followers|Followers)/)?.[1] || null;
    const following = text.match(/(\d[\d,\.]+\s*[KkMm]?)\s*(following|Following)/)?.[1] || null;
    const posts     = text.match(/(\d[\d,\.]+\s*[KkMm]?)\s*(posts|Posts)/)?.[1] || null;
    const bio       = text.match(/Bio[:\s]+([^\n]{5,200})/i)?.[1]?.trim() || null;
    const location  = extractSelfReportedLocation(text);
    ...
    throw new Error("Empty/blocked response from direct page read");
  } catch (error) {
    // Strategy 2 — Instagram actively blocks most server-side readers. Fall
    // back to a public search-cache lookup (search engines index the page's
    // public meta description, which includes follower/post counts) instead
    // of just surfacing the raw HTTP error.
    try {
      ...
      const blocked = isBlockedError(error.message);
      return {
        platform: "Instagram",
        status: blocked ? "blocked" : "not_found",
        title: `@${username}`,
        url,
        snippet: blocked
          ? "Instagram is actively blocking automated readers for this profile (common anti-scraping response). Open the link to verify manually."
          : "No public profile data could be confirmed for this username.",
...
async function scrapeLinkedIn(username) {
  ...
  try {
    ...
    const result = classifyReaderResponse(text, {
      blockedPatterns: [/sign in to linkedin|join linkedin|welcome back|authwall/i],
      notFoundPatterns: [/this page doesn.?t exist/i],
    });
    const headline   = result.snippet.match(/Headline[:\s]+([^\n]{5,200})/i)?.[1]?.trim() || null;
    const location   = result.snippet.match(/Location[:\s]+([^\n]{3,100})/i)?.[1]?.trim() || null;
    const connections= result.snippet.match(/(\d[\d,+]+)\s*(connections|followers)/i)?.[1] || null;
    return {
      platform: "LinkedIn",
      status: result.status,
      title: username,
      ...
      snippet: result.status === "found"
        ? result.snippet
        : result.status === "blocked"
          ? "LinkedIn requires login to render most profiles (authwall) — open the link to verify manually."
          : "No public LinkedIn profile could be confirmed for this username.",
      metadata: {
    ...
    return {
      ...
      title: username,
      url,
      snippet: `LinkedIn crawler failed: ${error.message}. Open the link to verify manually.`,
      extractor: "Jina Reader",
    };
  }
}
...
export async function runPublicOsintInvestigation({ target, type }) {
  ...
  if (username) {
    ...
    // Gemini-grounded corroboration: for platforms our direct scraper got
    // blocked on (not the ones genuinely "not found"), ask Gemini's built-in
    // Google Search grounding whether public info is independently indexed.
    // Skips silently if no Gemini key is configured.
    const [corrobResult, wmnResult] = await Promise.allSettled([
      getGeminiApiKey() ? corroborateBlockedFindings(platformFindings, username) : Promise.resolve(null),
      whatsmynameLookup(username),
    ]);
    if (corrobResult.status === "fulfilled" && getGeminiApiKey()) {
    ...
    } else if (corrobResult.status === "rejected" && getGeminiApiKey()) {
      logs.push({ time: nowTime(), level: "warn", msg: `Gemini corroboration pass failed: ${corrobResult.reason?.message}` });
    }
    if (wmnResult.status === "fulfilled" && wmnResult.value.length) {
      const wmn = wmnResult.value;
  ...
  const crawlSettled = await Promise.allSettled(urlsToFetch.map((url) => scrapePublicUrl(url)));
  ...
  if (gemini?.sources?.length) {
    ...
    const urlsToFetch2 = geminiUrls.filter((url) => !crawledPages.some((p) => p.url === url));
    const crawlSettled2 = await Promise.allSettled(urlsToFetch2.map((url) => scrapePublicUrl(url)));
    crawlSettled2.forEach((r, i) => {
      if (r.status === "fulfilled") crawledPages.push(r.value);
      else crawlErrors.push(`${urlsToFetch2[i]}: ${r.reason?.message || "failed"}`);
    });
  }
