...
export function analyzeContent(investigation) {
  const baseFindings = (investigation?.findings || []).filter(f => f?.snippet);
  ...
  const igFindings = igPosts
    .map(p => {
      const snippet = [p.caption, (p.hashtags || []).map(h => "#" + h).join(" ")].filter(Boolean).join(" ").trim();
      if (!snippet) return null;
      return { platform: "instagram", source: "instagram", snippet, value: p.shortCode || p.id };
    })
    .filter(Boolean);
  ...
  const topPosts = igPosts.slice(0, 3).map(p => ({
    id:        p.id ?? p.shortCode,
    image:     p.displayUrl || p.thumbnailSrc || p.thumbnailUrl || p.imageUrl
    ...
    likes:     p.likesCount ?? p.likes ?? null,
    comments:  p.commentsCount ?? p.commentsNumber ?? p.comments ?? null,
