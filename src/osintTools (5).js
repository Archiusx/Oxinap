...
async function apifyRunAndWait(actorId, input, token, maxWaitMs = 90_000) {
...
async function scrapeGoogleMapsReviews(locationQuery, reviewerHint, token) {
  ...
  let items;
  try {
    items = await apifyRunAndWait("Xb8osYTtOjlsgI6k9", input, token, 90_000);
  } catch (err) {
    return {
...
export function detectCryptoChain(address) {
  if (!address || typeof address !== "string") return null;
  const addr = address.trim();
  // Bitcoin Legacy (P2PKH/P2SH): starts with 1 or 3
  if (/^[13][A-HJ-NP-Za-km-z1-9]{25,34}$/.test(addr)) return "BTC";
  // Bitcoin Bech32 (SegWit): bc1...
  if (/^bc1[ac-hj-np-z02-9]{6,87}$/i.test(addr)) return "BTC";
  // Ethereum / EVM (ETH, BNB, MATIC, etc.): 0x + 40 hex
  if (/^0x[0-9a-fA-F]{40}$/.test(addr)) return "ETH";
  // Tron (TRX): T + 33 base58 chars
  if (/^T[A-Za-z1-9]{33}$/.test(addr)) return "TRX";
  // Litecoin: L, M, or ltc1
  if (/^[LM][A-HJ-NP-Za-km-z1-9]{26,33}$/.test(addr)) return "LTC";
  if (/^ltc1[ac-hj-np-z02-9]{6,87}$/i.test(addr)) return "LTC";
  // Dogecoin: D + 33 chars
  if (/^D[5-9A-HJ-NP-Za-km-z]{33}$/.test(addr)) return "DOGE";
  // Solana: base58 32-44 chars (no 0/O/I/l)
  if (/^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(addr) && addr.length >= 32 && addr.length <= 44) return "SOL";
  // XRP: starts with r
  if (/^r[0-9A-HJ-NP-Za-km-z]{24,34}$/.test(addr)) return "XRP";
  return null;
}

export function isCryptoAddress(address) {
  return Boolean(detectCryptoChain(address?.trim()));
}

// Known exchange/service hot wallet labels (subset of well-known addresses)
const KNOWN_WALLET_LABELS = {
  "1NDyJtNTjmwk5xPNhjgAMu4HDHigtobu1s": { label: "Binance Hot Wallet", type: "exchange" },
  "3E5LefFFzvnTTN8ADHU8KDTFCCyfT69aZD": { label: "Binance Cold Wallet", type: "exchange" },
  "0x3f5CE5FBFe3E9af3971dD833D26bA9b5C936f0bE": { label: "Binance", type: "exchange" },
  "0xD551234Ae421e3BCBA99A0Da6d736074f22192FF": { label: "Binance 2", type: "exchange" },
  "0x4E9ce36E442e55EcD9025B9a6E0D88485d628A67": { label: "Binance 3", type: "exchange" },
  "0x564286362092D8e7936f0549571a803B203aAceD": { label: "Binance 4", type: "exchange" },
  "0x21a31Ee1afC51d94C2eFcCAa2092aD1028285549": { label: "Binance 5", type: "exchange" },
  "0xBE0eB53F46cd790Cd13851d5EFf43D12404d33E8": { label: "Binance 6", type: "exchange" },
  "0xa7efae728d369157a1d8074d3c6df5d1b2dae369": { label: "Kraken", type: "exchange" },
  "0x2910543af39aba0cd09dbb2d50200b3e800a63d2": { label: "Kraken 2", type: "exchange" },
  "0x59a5208B32e627891C389EaFDC7421b2CA674A7": { label: "Coinbase", type: "exchange" },
  "1FzWLkAahHooV3kzTgyx6qsswXJ6sCXkSR": { label: "Coinbase Hot Wallet", type: "exchange" },
  "1GmkX3J8a7PCbAgBLTc1WGC8B9LnKLHFoe": { label: "OKEx", type: "exchange" },
};

function formatCrypto(value, decimals = 8) {
  if (value === null || value === undefined) return "Unknown";
  return (value / Math.pow(10, decimals)).toFixed(decimals).replace(/\.?0+$/, "") || "0";
}

function formatUSD(satoshiValue, btcPrice = null) {
  if (!btcPrice || satoshiValue === null) return null;
  const usd = (satoshiValue / 1e8) * btcPrice;
  return usd >= 1 ? `$${usd.toLocaleString(undefined, { maximumFractionDigits: 2 })}` : `$${usd.toFixed(6)}`;
}

async function lookupBTCAddress(address) {
  // Primary: Blockstream API (CORS-enabled, free)
  try {
    const [addrData, txsData] = await Promise.all([
      fetchJson(`https://blockstream.info/api/address/${encodeURIComponent(address)}`, {}, 8000),
      fetchJson(`https://blockstream.info/api/address/${encodeURIComponent(address)}/txs`, {}, 8000).catch(() => []),
    ]);

    const funded   = addrData.chain_stats?.funded_txo_sum ?? addrData.mempool_stats?.funded_txo_sum ?? 0;
    const spent    = addrData.chain_stats?.spent_txo_sum  ?? addrData.mempool_stats?.spent_txo_sum  ?? 0;
    const balance  = funded - spent;
    const txCount  = (addrData.chain_stats?.tx_count ?? 0) + (addrData.mempool_stats?.tx_count ?? 0);
    const balanceBTC = (balance / 1e8).toFixed(8).replace(/\.?0+$/, "") || "0";

    const knownLabel = KNOWN_WALLET_LABELS[address];
    const recentTxs  = Array.isArray(txsData) ? txsData.slice(0, 5) : [];
    const firstSeen  = recentTxs.length > 0
      ? new Date((recentTxs[recentTxs.length - 1]?.status?.block_time ?? 0) * 1000).toLocaleDateString()
      : "Unknown";
    const lastSeen   = recentTxs.length > 0
      ? new Date((recentTxs[0]?.status?.block_time ?? 0) * 1000).toLocaleDateString()
      : "Unknown";

    return {
      platform: "Bitcoin (BTC)",
      status:   "found",
      title:    `BTC: ${address.slice(0, 8)}…${address.slice(-6)}`,
      url:      `https://blockstream.info/address/${address}`,
      snippet:  `Balance: ${balanceBTC} BTC · Transactions: ${txCount}${knownLabel ? ` · 🏦 ${knownLabel.label}` : ""}`,
      metadata: {
        chain:         "Bitcoin (BTC)",
        address:       address,
        balance:       `${balanceBTC} BTC`,
        total_received:`${formatCrypto(funded)} BTC`,
        total_sent:    `${formatCrypto(spent)} BTC`,
        tx_count:      String(txCount),
        first_seen:    firstSeen,
        last_seen:     lastSeen,
        known_entity:  knownLabel ? `${knownLabel.label} (${knownLabel.type})` : "Unknown / Private",
      ...
      recentTxs: recentTxs.map(tx => ({
        ...
        value:   tx.vout?.reduce((s, o) => s + (o.value || 0), 0),
        url:     `https://blockstream.info/tx/${tx.txid}`,
      })),
      extractor: "Blockstream API",
    };
  } catch (err) {
    // Fallback: blockchain.info
    try {
      const data = await fetchJson(`https://blockchain.info/rawaddr/${encodeURIComponent(address)}?limit=5&cors=true`, {}, 8000);
      const balanceBTC = ((data.final_balance || 0) / 1e8).toFixed(8).replace(/\.?0+$/, "") || "0";
      const knownLabel = KNOWN_WALLET_LABELS[address];
      return {
        platform: "Bitcoin (BTC)",
        status:   "found",
        ...
        snippet:  `Balance: ${balanceBTC} BTC · Transactions: ${data.n_tx || 0}${knownLabel ? ` · 🏦 ${knownLabel.label}` : ""}`,
        metadata: {
          chain:         "Bitcoin (BTC)",
          address:       address,
          balance:       `${balanceBTC} BTC`,
          total_received:`${formatCrypto(data.total_received || 0)} BTC`,
          total_sent:    `${formatCrypto(data.total_sent || 0)} BTC`,
          tx_count:      String(data.n_tx || 0),
          first_seen:    "See explorer",
          known_entity:  knownLabel ? `${knownLabel.label} (${knownLabel.type})` : "Unknown / Private",
          explorer:      `https://blockchain.info/address/${address}`,
        },
        extractor: "blockchain.info API",
      };
    } catch (e2) {
      return {
        platform: "Bitcoin (BTC)",
        ...
        title:    `BTC: ${address.slice(0, 8)}…${address.slice(-6)}`,
        url:      `https://blockstream.info/address/${address}`,
        snippet:  `BTC lookup failed: ${e2.message}. Open explorer link to check manually.`,
        metadata: { chain: "Bitcoin (BTC)", address },
        extractor: "Blockstream / blockchain.info",
      };
    }
  }
}
...
async function lookupETHAddress(address) {
  ...
  try {
    const [balRes, txRes, internalRes] = await Promise.all([
      fetchJson(`${baseUrl}?module=account&action=balance&address=${encodeURIComponent(address)}&tag=latest${keyParam}`, {}, 8000),
      fetchJson(`${baseUrl}?module=account&action=txlist&address=${encodeURIComponent(address)}&startblock=0&endblock=99999999&page=1&offset=10&sort=desc${keyParam}`, {}, 8000).catch(() => null),
      fetchJson(`${baseUrl}?module=account&action=tokentx&address=${encodeURIComponent(address)}&page=1&offset=5&sort=desc${keyParam}`, {}, 8000).catch(() => null),
    ]);

    if (balRes.status === "0" && balRes.message === "NOTOK") {
      throw new Error(balRes.result || "Etherscan rate limit or invalid address");
    }

    const ethBalance  = ((Number(balRes.result) || 0) / 1e18).toFixed(6).replace(/\.?0+$/, "") || "0";
    const txList      = Array.isArray(txRes?.result) ? txRes.result : [];
    const tokenTxList = Array.isArray(internalRes?.result) ? internalRes.result : [];
    const txCount     = txList.length;

    const firstTx  = txList.length > 0 ? txList[txList.length - 1] : null;
    const lastTx   = txList.length > 0 ? txList[0] : null;
    const firstSeen = firstTx ? new Date(Number(firstTx.timeStamp) * 1000).toLocaleDateString() : "Unknown";
    const lastSeen  = lastTx  ? new Date(Number(lastTx.timeStamp)  * 1000).toLocaleDateString() : "Unknown";

    // Unique tokens interacted with
    const tokens = [...new Set(tokenTxList.map(t => t.tokenSymbol).filter(Boolean))].slice(0, 8);
    const knownLabel = KNOWN_WALLET_LABELS[address] || KNOWN_WALLET_LABELS[address.toLowerCase()];

    // Detect contract vs EOA
    const isContract = txList.some(t => !t.input || t.input === "0x") ? false : null;

    return {
      platform: "Ethereum (ETH)",
      status:   "found",
      title:    `ETH: ${address.slice(0, 8)}…${address.slice(-6)}`,
      url:      `https://etherscan.io/address/${address}`,
      snippet:  `Balance: ${ethBalance} ETH · Transactions: ${txCount}${tokens.length ? ` · Tokens: ${tokens.join(", ")}` : ""}${knownLabel ? ` · 🏦 ${knownLabel.label}` : ""}`,
      metadata: {
        chain:         "Ethereum (ETH)",
        address:       address,
      ...
      extractor: "Etherscan API",
    };
  } catch (err) {
    // Fallback: try Ankr free public RPC (no API key needed)
    try {
      const ankrRes = await fetchJson("https://rpc.ankr.com/eth", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ jsonrpc: "2.0", method: "eth_getBalance", params: [address, "latest"], id: 1 }),
      }, 8000);
      const rawBal = ankrRes?.result;
      const ethBalance = rawBal ? (parseInt(rawBal, 16) / 1e18).toFixed(6).replace(/\.?0+$/, "") || "0" : "Unknown";
      return {
        platform: "Ethereum (ETH)",
        status:   ethBalance !== "Unknown" ? "found" : "blocked",
        title:    `ETH: ${address.slice(0, 8)}…${address.slice(-6)}`,
        url:      `https://etherscan.io/address/${address}`,
        snippet:  `Balance: ${ethBalance} ETH (via Ankr RPC). Open Etherscan for full tx history.`,
        metadata: { chain: "Ethereum (ETH)", address, balance: `${ethBalance} ETH`, explorer: `https://etherscan.io/address/${address}`, etherscan: `https://etherscan.io/address/${address}` },
        recentTxs: [],
        extractor: "Ankr Public RPC (fallback)",
      };
    } catch {
      return {
        platform: "Ethereum (ETH)",
        ...
        title:    `ETH: ${address.slice(0, 8)}…${address.slice(-6)}`,
        url:      `https://etherscan.io/address/${address}`,
        snippet:  `ETH lookup failed: ${err.message}. Open Etherscan link to check manually.`,
        metadata: { chain: "Ethereum (ETH)", address },
        extractor: "Etherscan API",
      };
    }
  }
...
async function lookupTRXAddress(address) {
  try {
    const data = await fetchJson(`https://apilist.tronscanteam.com/api/account?address=${encodeURIComponent(address)}`, {}, 8000);
    const balanceTRX = ((data.balance || 0) / 1e6).toFixed(2);
    const txCount    = data.transactions || data.totalTransactionCount || 0;
    const bandwidth  = data.bandwidth?.freeNetLimit || 0;
    const energy     = data.bandwidth?.energyLimit  || 0;
    const tokenBalances = (data.tokenBalances || []).slice(0, 6).map(t => `${t.tokenAbbr || t.tokenName}: ${(Number(t.balance) / Math.pow(10, t.tokenDecimal || 6)).toFixed(2)}`);

    return {
      platform: "Tron (TRX)",
      status:   "found",
      title:    `TRX: ${address.slice(0, 8)}…${address.slice(-6)}`,
      url:      `https://tronscan.org/#/address/${address}`,
      snippet:  `Balance: ${balanceTRX} TRX · Transactions: ${txCount}${tokenBalances.length ? ` · Tokens: ${tokenBalances.slice(0, 3).join(", ")}` : ""}`,
      metadata: {
        chain:       "Tron (TRX)",
        address:     address,
        ...
        tokens:      tokenBalances.join(" | ") || "None",
        explorer:    `https://tronscan.org/#/address/${address}`,
      },
      extractor: "Tronscan API",
    };
  } catch (err) {
    // Fallback: trongrid public API
    try {
      const tg = await fetchJson(`https://api.trongrid.io/v1/accounts/${encodeURIComponent(address)}`, {}, 8000);
      const acc = tg?.data?.[0];
      const balanceTRX = acc ? ((acc.balance || 0) / 1e6).toFixed(2) : "Unknown";
      return {
        platform: "Tron (TRX)",
        status:   acc ? "found" : "blocked",
        title:    `TRX: ${address.slice(0, 8)}…${address.slice(-6)}`,
        url:      `https://tronscan.org/#/address/${address}`,
        snippet:  `Balance: ${balanceTRX} TRX (via Trongrid). Open Tronscan for full history.`,
        metadata: { chain: "Tron (TRX)", address, balance: `${balanceTRX} TRX`, explorer: `https://tronscan.org/#/address/${address}` },
        extractor: "Trongrid API (fallback)",
      };
    } catch {
      return {
        platform: "Tron (TRX)", status: "blocked",
        title: `TRX: ${address.slice(0, 8)}…${address.slice(-6)}`,
        url: `https://tronscan.org/#/address/${address}`,
        snippet: `TRX lookup failed: ${err.message}.`,
        metadata: { chain: "Tron (TRX)", address }, extractor: "Tronscan API",
      };
    }
  }
}

async function lookupLTCAddress(address) {
  try {
    const data = await fetchJson(`https://api.blockcypher.com/v1/ltc/main/addrs/${encodeURIComponent(address)}/balance`, {}, 8000);
    const balLTC = ((data.balance || 0) / 1e8).toFixed(8).replace(/\.?0+$/, "") || "0";
    return {
      platform: "Litecoin (LTC)",
      status:   "found",
      title:    `LTC: ${address.slice(0, 8)}…${address.slice(-6)}`,
      url:      `https://live.blockcypher.com/ltc/address/${address}/`,
      snippet:  `Balance: ${balLTC} LTC · Transactions: ${data.n_tx || 0}`,
      metadata: {
        chain:         "Litecoin (LTC)",
        address:       address,
        balance:       `${balLTC} LTC`,
        total_received: `${formatCrypto(data.total_received || 0)} LTC`,
        tx_count:      String(data.n_tx || 0),
        explorer:      `https://live.blockcypher.com/ltc/address/${address}/`,
      },
      extractor: "BlockCypher API",
    };
  } catch (err) {
    try {
      const d = await fetch(`https://sochain.com/api/v2/get_address_balance/LTC/${encodeURIComponent(address)}`, { signal: AbortSignal.timeout(7000) });
      const j = await d.json();
      const bal = j?.data?.confirmed_balance || "Unknown";
      return {
        platform: "Litecoin (LTC)", status: bal !== "Unknown" ? "found" : "blocked",
        title: `LTC: ${address.slice(0, 8)}…${address.slice(-6)}`,
        url: `https://live.blockcypher.com/ltc/address/${address}/`,
        snippet: `Balance: ${bal} LTC (via SoChain).`,
        metadata: { chain: "Litecoin (LTC)", address, balance: `${bal} LTC`, explorer: `https://live.blockcypher.com/ltc/address/${address}/` },
        extractor: "SoChain API (fallback)",
      };
    } catch {
      return {
        platform: "Litecoin (LTC)", status: "blocked",
        title: `LTC: ${address.slice(0, 8)}…${address.slice(-6)}`,
        url: `https://live.blockcypher.com/ltc/address/${address}/`,
        snippet: `LTC lookup failed: ${err.message}.`,
        metadata: { chain: "Litecoin (LTC)", address }, extractor: "BlockCypher API",
      };
    }
  }
}

async function lookupDOGEAddress(address) {
  try {
    const data = await fetchJson(`https://api.blockcypher.com/v1/doge/main/addrs/${encodeURIComponent(address)}/balance`, {}, 8000);
    const balDOGE = ((data.balance || 0) / 1e8).toFixed(2);
    return {
      platform: "Dogecoin (DOGE)",
      status:   "found",
      title:    `DOGE: ${address.slice(0, 8)}…${address.slice(-6)}`,
      url:      `https://live.blockcypher.com/doge/address/${address}/`,
      snippet:  `Balance: ${balDOGE} DOGE · Transactions: ${data.n_tx || 0}`,
      metadata: {
        chain:    "Dogecoin (DOGE)", address,
        balance:  `${balDOGE} DOGE`,
        tx_count: String(data.n_tx || 0),
        explorer: `https://live.blockcypher.com/doge/address/${address}/`,
      },
      extractor: "BlockCypher API",
    };
  } catch (err) {
    try {
      const d = await fetch(`https://sochain.com/api/v2/get_address_balance/DOGE/${encodeURIComponent(address)}`, { signal: AbortSignal.timeout(7000) });
      const j = await d.json();
      const bal = j?.data?.confirmed_balance || "Unknown";
      return {
        platform: "Dogecoin (DOGE)", status: bal !== "Unknown" ? "found" : "blocked",
        title: `DOGE: ${address.slice(0, 8)}…${address.slice(-6)}`,
        url: `https://live.blockcypher.com/doge/address/${address}/`,
        snippet: `Balance: ${bal} DOGE (via SoChain).`,
        metadata: { chain: "Dogecoin (DOGE)", address, balance: `${bal} DOGE`, explorer: `https://live.blockcypher.com/doge/address/${address}/` },
        extractor: "SoChain API (fallback)",
      };
    } catch {
      return {
        platform: "Dogecoin (DOGE)", status: "blocked",
        title: `DOGE: ${address.slice(0, 8)}…${address.slice(-6)}`,
        url: `https://live.blockcypher.com/doge/address/${address}/`,
        snippet: `DOGE lookup failed: ${err.message}.`,
        metadata: { chain: "Dogecoin (DOGE)", address }, extractor: "BlockCypher API",
      };
    }
  }
}

async function lookupSOLAddress(address) {
  // Use Solana public JSON-RPC
  try {
    const rpc = await fetchJson("https://api.mainnet-beta.solana.com", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        jsonrpc: "2.0", id: 1,
    ...
    const sigRes = await fetchJson("https://api.mainnet-beta.solana.com", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        jsonrpc: "2.0", id: 2,
    ...
    return {
      platform: "Solana (SOL)",
      status:   "found",
      title:    `SOL: ${address.slice(0, 8)}…${address.slice(-6)}`,
      url:      `https://solscan.io/account/${address}`,
      snippet:  `Balance: ${solBalance} SOL · Recent signatures: ${sigs.length}`,
      metadata: {
        chain:    "Solana (SOL)", address,
        balance:  `${solBalance} SOL`,
        tx_count: `${sigs.length}+ (recent)`,
        explorer: `https://solscan.io/account/${address}`,
        solscan:  `https://solscan.io/account/${address}`,
      },
      extractor: "Solana JSON-RPC",
    };
  } catch (err) {
    // Fallback: Ankr public SOL RPC
    try {
      const ankr = await fetchJson("https://rpc.ankr.com/solana", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "getBalance", params: [address, { commitment: "finalized" }] }),
      }, 7000);
      const sol = ankr?.result?.value != null ? (ankr.result.value / 1e9).toFixed(4) : "Unknown";
      return {
        platform: "Solana (SOL)", status: sol !== "Unknown" ? "found" : "blocked",
        title: `SOL: ${address.slice(0, 8)}…${address.slice(-6)}`,
        url: `https://solscan.io/account/${address}`,
        snippet: `Balance: ${sol} SOL (via Ankr RPC). Open Solscan for full history.`,
        metadata: { chain: "Solana (SOL)", address, balance: `${sol} SOL`, explorer: `https://solscan.io/account/${address}` },
        extractor: "Ankr SOL RPC (fallback)",
      };
    } catch {
      return {
        platform: "Solana (SOL)", status: "blocked",
        title: `SOL: ${address.slice(0, 8)}…${address.slice(-6)}`,
        url: `https://solscan.io/account/${address}`,
        snippet: `SOL lookup failed: ${err.message}.`,
        metadata: { chain: "Solana (SOL)", address }, extractor: "Solana RPC",
      };
    }
  }
}

// OSINT cross-reference: search Gemini for wallet mentions in dark web / forums
async function cryptoGeminiSearch(address, chain) {
  if (!getGeminiApiKey()) return null;
  const prompt = `You are an OSINT blockchain analyst. Search for public information about this ${chain} wallet address: ${address}. 
Look for: mentions in public forums, social media, scam databases, exchange labels, blockchain explorers, news articles. 
Only report what you find in publicly indexed sources — no speculation.
Return concise findings: known entity label if any, risk flags, public mentions, associated usernames/emails if any appeared in public records.`;
  try {
    return await askGemini(prompt, 600);
  } catch { return null; }
}

// Main crypto wallet OSINT runner
export async function runCryptoWalletOsint({ address }) {
  const addr  = (address || "").trim();
  const chain = detectCryptoChain(addr);
  const logs  = [{ time: nowTime(), level: "info", msg: `Crypto wallet OSINT started for ${chain || "unknown chain"}: ${addr.slice(0, 12)}…` }];

  if (!addr || !chain) {
    throw new Error("Invalid or unrecognised wallet address. Supported: BTC, ETH, TRX, LTC, DOGE, SOL, XRP.");
  }

  logs.push({ time: nowTime(), level: "info", msg: `Detected chain: ${chain}. Querying blockchain explorer APIs…` });

  // 1. Chain-specific lookup
  const lookupFn = {
    BTC:  () => lookupBTCAddress(addr),
    ETH:  () => lookupETHAddress(addr),
    TRX:  () => lookupTRXAddress(addr),
    LTC:  () => lookupLTCAddress(addr),
    DOGE: () => lookupDOGEAddress(addr),
    SOL:  () => lookupSOLAddress(addr),
    XRP:  () => Promise.resolve({ platform: "XRP", status: "open_link", title: `XRP: ${addr.slice(0,8)}…`, url: `https://xrpscan.com/account/${addr}`, snippet: "Open XRP Scan to view account details.", metadata: { chain: "XRP", address: addr, explorer: `https://xrpscan.com/account/${addr}` }, extractor: "Manual" }),
  }[chain] || (() => Promise.resolve(null));

  // 2. Run blockchain lookup + Gemini search concurrently
  // Note: runPublicReaderSearch removed from crypto — it triggers heavy crawling that crashes mobile browsers
  const [chainResult, geminiResult] = await Promise.allSettled([
    lookupFn(),
    cryptoGeminiSearch(addr, chain),
  ]);
  ...
  if (chainResult.status === "fulfilled" && chainResult.value) {
    primaryResult = chainResult.value;
    findings.push(primaryResult);
    const lvl = primaryResult.status === "found" ? "success" : primaryResult.status === "blocked" ? "warn" : "info";
    logs.unshift({ time: nowTime(), level: lvl, msg: `${primaryResult.platform}: ${primaryResult.snippet.slice(0, 100)}` });
  }
  ...
  if (geminiResult.status === "fulfilled" && geminiResult.value) {
    ...
    if (gemini.enabled) {
      logs.unshift({ time: nowTime(), level: "success", msg: "Gemini grounded search found public mentions of this wallet." });
    }
  }

  // 4. Web search results (disabled for crypto — use explorer links instead)
  const crawledPages = [];

  // 5. Risk scoring
  const knownLabel = KNOWN_WALLET_LABELS[addr] || KNOWN_WALLET_LABELS[addr.toLowerCase()];
  const hasBalance = primaryResult?.metadata?.balance && primaryResult.metadata.balance !== "0";
  const txCount    = parseInt(primaryResult?.metadata?.tx_count || "0", 10);
  const confidence = primaryResult?.status === "found" ? Math.min(95, 50 + (txCount > 100 ? 30 : txCount > 10 ? 15 : 5) + (gemini?.enabled ? 15 : 0)) : 20;
  const risk       = confidence >= 80 ? "high" : confidence >= 50 ? "medium" : "low";
  ...
  return {
    ...
    status:      "Completed",
    risk,
    platforms:   [primaryResult?.platform || chain].filter(Boolean),
    startedAt:   new Date().toISOString(),
    finding:     primaryResult,
    findings,
    knownLabel,
    crawledPages,
    gemini,
    logs,
    metadata: [
      { key: "Wallet Address", value: addr },
      { key: "Blockchain",     value: chain },
      { key: "Known Entity",   value: knownLabel ? `${knownLabel.label} (${knownLabel.type})` : "Unknown / Private" },
      { key: "Balance",        value: primaryResult?.metadata?.balance || "Unknown" },
      { key: "Transactions",   value: primaryResult?.metadata?.tx_count || "Unknown" },
      { key: "First Seen",     value: primaryResult?.metadata?.first_seen || "Unknown" },
      { key: "Last Active",    value: primaryResult?.metadata?.last_seen  || "Unknown" },
    ],
    stats: { confidence },
  };
}

// Crypto explorer links for a given address+chain
export function getCryptoExplorerLinks(address, chain) {
  const links = {
    BTC: [
      { name: "Blockstream",      url: `https://blockstream.info/address/${address}` },
      { name: "Mempool.space",    url: `https://mempool.space/address/${address}` },
      { name: "Blockchain.info",  url: `https://www.blockchain.com/explorer/addresses/btc/${address}` },
      { name: "BTC.com",          url: `https://btc.com/${address}` },
    ],
    ETH: [
      { name: "Etherscan",        url: `https://etherscan.io/address/${address}` },
      { name: "DeBank",           url: `https://debank.com/profile/${address}` },
      { name: "Arkham",           url: `https://platform.arkhamintelligence.com/explorer/address/${address}` },
      { name: "Zapper",           url: `https://zapper.xyz/account/${address}` },
    ],
    TRX: [
      { name: "Tronscan",         url: `https://tronscan.org/#/address/${address}` },
    ],
    LTC: [
      { name: "BlockCypher",      url: `https://live.blockcypher.com/ltc/address/${address}/` },
    ],
    DOGE: [
      { name: "BlockCypher",      url: `https://live.blockcypher.com/doge/address/${address}/` },
      { name: "Dogechain",        url: `https://dogechain.info/address/${address}` },
    ],
    SOL: [
      { name: "Solscan",          url: `https://solscan.io/account/${address}` },
      { name: "Solana Explorer",  url: `https://explorer.solana.com/address/${address}` },
    ],
    XRP: [
      { name: "XRPScan",          url: `https://xrpscan.com/account/${address}` },
      { name: "Bithomp",          url: `https://bithomp.com/explorer/${address}` },
    ],
  };
  return links[chain] || [];
}

// OSINT tools list for crypto
export const CRYPTO_OSINT_TOOLS = [
  { name: "Chainabuse",           url: "https://www.chainabuse.com/",                  note: "Report/check crypto addresses for scams and abuse across Bitcoin, ETH, TRON." },
  { name: "Crystal Blockchain",   url: "https://crystalblockchain.com/",               note: "Enterprise blockchain analytics — AML/KYC risk scoring for wallet addresses." },
  { name: "Chainalysis KYT",      url: "https://www.chainalysis.com/chainalysis-kyt/", note: "Industry-standard transaction monitoring and risk scoring." },
  { name: "Elliptic",             url: "https://www.elliptic.co/",                     note: "Crypto risk analytics; free wallet screening on request." },
  { name: "MistTrack",            url: "https://misttrack.io/",                        note: "Free crypto address OSINT: entity labels, risk score, asset tracking." },
  { name: "OKLink",               url: "https://www.oklink.com/",                      note: "Multi-chain explorer with entity labels, risk alerts, and address clustering." },
  { name: "Breadcrumbs.app",      url: "https://www.breadcrumbs.app/",                 note: "Free visual blockchain investigation tool for ETH and EVM chains." },
  { name: "Arkham Intelligence",  url: "https://platform.arkhamintelligence.com/",     note: "On-chain entity identification and fund-flow tracing (ETH, BTC, others)." },
  { name: "Nansen",               url: "https://www.nansen.ai/",                       note: "Wallet labels and smart-money tracking on Ethereum." },
  { name: "BitcoinAbuse",         url: "https://www.bitcoinabuse.com/",                note: "Public database of BTC addresses reported for ransomware, fraud, scams." },
  { name: "Walletexplorer",       url: "https://www.walletexplorer.com/",              note: "Identifies which exchange or service owns a Bitcoin wallet cluster." },
  { name: "OFAC SDN Search",      url: "https://sanctionssearch.ofac.treas.gov/",      note: "US Treasury sanctions list — check if address/entity is sanctioned." },
  { name: "DeBank",               url: "https://debank.com/",                          note: "Free DeFi portfolio explorer for ETH and EVM addresses." },
  { name: "Blockchair",           url: "https://blockchair.com/",                      note: "Privacy-friendly multi-chain explorer supporting BTC, ETH, SOL, DOGE, and many more." },
];
